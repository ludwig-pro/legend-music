...
class ObservablePersistExpoFS implements ObservablePersistPlugin {
	...
	public initialize(_configOptions: ObservablePersistPluginOptions) {
		...
		try {
			...
			console.error(
				"[legend-state] ObservablePersistReacExpoFS failed to initialize",
				e,
			);
		}
	}
	...
	public loadTable(table: string) {
		if (this.data[table] === undefined) {
			try {
				const mainTableFile = new FileSystemNext.File(
					this.directory,
				...
				console.error(
					"[legend-state] ObservablePersistReacExpoFS failed to load table",
					table,
					err,
				);
			}
	...
	private async saveDebounced(table: string) {
		...
		const file = new FileSystemNext.File(
			this.directory,
			`${table}.${this.extension}`,
		);
...
export function observablePersistExpoFS(
	configuration: ExpoFSPersistPluginOptions = { format: "json" },
) {
